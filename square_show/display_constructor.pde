void displayFilters(){ // putting all 17 requirements at the top of the graph
  ...
  String[] words = {"Date","Carrier","Fl num","Origin","City Name","Origin State","Orig Wac",
                    "Destination","Dest City","Dest State","Dest Wac","CRS Dep t","Dep time",
                    "CRS arr t","Arr time","Cancelled","Diverted","Distance"};
  for (int i = 0; i < words.length; i++) { // go through each word
  fill(0);
  textSize(15);
  text(words[i], i*77.7, 300); // 1400 our screen length /18 is 77.7. 
